...
class Department:
    @staticmethod
    def create(organization_id: int, name: str, slug: str, department_context: Optional[str] = None) -> int:
        """Creates a new department linked to an organization with optional context."""
        ...
        query = """
            INSERT INTO departments (organization_id, name, slug, department_context)
            VALUES (?, ?, ?, ?);
        """
        return DatabaseManager.execute_update(query, (organization_id, name, slug, department_context))
    ...
    def update(dept_id: int, updates: Dict[str, Any]) -> bool:
        allowed_keys = {"name", "slug", "status", "department_context"}
    ...
    def soft_delete(dept_id: int) -> bool:
        ...
        success = DatabaseManager.execute_update(query, (dept_id,)) > 0
        if success:
